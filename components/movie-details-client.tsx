...
export default function MovieDetailsClient({
  ...
  const releaseDate = movie.release_date ? new Date(movie.release_date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : "Release date unknown"
  ...
  const isInContinueWatching = movieProgress && movieProgress.progress && movieProgress.progress.watched > 0
    ...
    <>
      <div className="relative w-full min-h-[500px] md:min-h-[600px]">
        <div className="absolute inset-0">
          {backdropPath ? (
            <img src={backdropPath} alt={""} className="object-cover absolute top-0 left-0 w-full h-full" />
          ) : (
          ...
          <div className="container px-4">
            <div className="grid items-start gap-8 md:grid-cols-[300px_1fr]">
              <div className="relative hidden overflow-hidden rounded-lg shadow-lg md:block aspect-[2/3]">
                {posterPath ? (
                  <img src={posterPath} alt={movie.title || "Movie poster"} className="object-cover absolute top-0 left-0 w-full h-full" />
                ) : (
              ...
              <div className="space-y-4">
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl lg:text-5xl">{movie.title || "Untitled Movie"}</h1>
                <div className="flex flex-wrap gap-2">
                  {genres.map((genre) => (
                    <Link key={genre.id} href={`/discover?media_type=movie&with_genres=${genre.id}`}>
                ...
                {director && (
                  <div className="text-sm">
                    ...
                    <Link href={`/person/${director.id}`} className="hover:underline">{director.name}</Link>
                  </div>
                )}
                <p className="text-muted-foreground max-w-prose">{movie.overview || "No overview available."}</p>
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  ...
                  <DownloadModal mediaType="movie" tmdbId={id} title={movie.title || "Movie"} />
                  <WatchlistDropdown mediaId={id} mediaType="movie" title={movie.title || "Movie"} posterPath={movie.poster_path} genres={movie.genres} />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="container px-4 mt-8">
        <Tabs defaultValue="cast" className="space-y-8" onValueChange={handleTabChange}>
          <TabsList>
          ...
          <TabsContent value="similar" className="space-y-6">
            ...
            <Separator />
            {similarMovies.length > 0 ? <MediaRow items={similarMovies.slice(0, 8).map((i) => ({ ...i, media_type: "movie" }))} /> : <div className="p-8 text-center border rounded-lg text-muted-foreground">No similar movies available.</div>}
          </TabsContent>
