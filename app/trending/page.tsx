      ...
      <>
        {trending.results?.length > 0 ? (
          <>
            ...
            <MediaPagination currentPage={Number(page)} totalPages={Math.min(trending.total_pages || 1, 500)} totalResults={trending.total_results || 0} />
          </>
        ) : (
          <div className="text-center py-12"><p className="text-muted-foreground">No trending content found.</p></div>
        )}
      </>
    )
  } catch (error) {
    return <div className="text-center py-12"><p className="text-destructive">Error loading content.</p></div>
  }
}
